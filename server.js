...
app.post('/api/tasks/:taskId/assign', (req, res) => {
  try {
    ...
    const workloadIncrease = (task.estimated_hours || 40) / hoursPerWeek * 100;
    ...
    resource.availability = resource.current_workload >= 80 ? 'busy' : 'available';
...
app.post('/api/tasks/:taskId/unassign', (req, res) => {
  try {
    ...
    if (!task.assigned_resource) {
      return res.status(400).json({ success: false, error: 'Task is not assigned' });
    }
    ...
    if (resource) {
      ...
      const hoursPerWeek = 40;
      const workloadDecrease = (task.estimated_hours || 40) / hoursPerWeek * 100;
      ...
      if (resource.assigned_tasks && resource.assigned_tasks.includes(taskId)) {
        resource.assigned_tasks = resource.assigned_tasks.filter(id => id !== taskId);
      }
    }
...
app.get('/api/stats', (req, res) => {
  try {
    ...
    // Average workload
    const avgWorkload = totalResources > 0
      ? resources.reduce((sum, r) => sum + r.current_workload, 0) / totalResources
      : 0;
